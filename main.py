...
async def main():
    ...
    crawler = AsyncCrawler(args.url, max_pages=args.max_pages, concurrency=args.concurrency)
    await crawler.crawl()
    crawler.report()
    
    db_saver = DBSaver(db_url=db_url)
    db_saver.save_to_db(crawler.get_internalLinks(), crawler.get_externalDomains())
    db_saver.close()
